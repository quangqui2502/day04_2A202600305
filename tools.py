...
def search_flights(origin: str, destination: str) -> str:
    ...
    if not flights:
        return f"Không tìm thấy chuyến bay từ {origin} đến {destination}. Hiện tại chúng tôi có các tuyến: Hà Nội ↔ Đà Nẵng, Hà Nội ↔ Phú Quốc, Hà Nội ↔ Hồ Chí Minh, Hồ Chí Minh ↔ Đà Nẵng, Hồ Chí Minh ↔ Phú Quốc."
...
def search_hotels(city: str, max_price_per_night: int = 99_999_999) -> str:
    ...
    if not hotels:
        ...
        return f"Không tìm thấy khách sạn tại {city}. Hiện có dữ liệu cho: {available}."
    ...
    lines = [f"Khách sạn tại {city} (giá ≤ {max_price_per_night:,}đ/đêm):\n".replace(",", ".")]
    for i, h in enumerate(filtered_sorted, 1):
        price_fmt = f"{h['price_per_night']:,}đ".replace(",", ".")
        lines.append(
            f"{i}. {h['name']} ({h['stars']}⭐) | {price_fmt}/đêm | "
    ...
    try:
        ...
        for item in expenses.split(","):
            ...
            if not item:
                continue
            if ":" not in item:
                return f"Lỗi định dạng: '{item}' phải theo dạng 'tên:số_tiền'. VD: 'vé_máy_bay:890000'"
            name, amount_str = item.split(":", 1)
            name = name.strip().replace("_", " ")
            try:
                amount = int(amount_str.strip())
            except ValueError:
                return f"Lỗi: '{amount_str.strip()}' không phải số hợp lệ cho khoản '{name}'."
        ...
        for name, amount in expense_dict.items():
            ...
            lines.append(f"  - {name.capitalize()}: {amount_fmt}")

        budget_fmt    = f"{total_budget:,}đ".replace(",", ".")
        total_fmt     = f"{total_expense:,}đ".replace(",", ".")
        remaining_fmt = f"{abs(remaining):,}đ".replace(",", ".")

        lines.append("  ---")
        lines.append(f"  Tổng chi: {total_fmt}")
        lines.append(f"  Ngân sách: {budget_fmt}")

        if remaining >= 0:
            lines.append(f"  Còn lại: {remaining_fmt} ✅")
        else:
            lines.append(f"  ⚠️ Vượt ngân sách {remaining_fmt}! Cần điều chỉnh kế hoạch.")
